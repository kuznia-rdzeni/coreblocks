...
* `-b`, `--backend` -- select a backend for the test suites selected by `-a`.
  ...
  - `cxxsim` -- utilizes Verilator with a testbench written in C++.

  The `cocotb` backend is selected by default.
  All three simulate the same core, defined once in `test/sim/core.py`:
  `pysim` elaborates it, while the other two build the Verilog generated from it.
