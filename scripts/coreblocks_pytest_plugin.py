"""
This module is a compatibility layer between run_tests.py and pytest to provide backward compatible syntax in run_tests.py.
It is implemented as pytest plugin, which provides following pytest hooks:
...
def pytest_addoption(parser : pytest.Parser):
    ...
    group.addoption("--coreblocks-test-name", action="store", type=str, help="Name or regexp in flatten format pointing to test to run.")
    group.addoption("--coreblocks-test-count", action="store", type=int, help="Number of tests to starts. If less than number of all selected tests, then starts only subset of them.")
