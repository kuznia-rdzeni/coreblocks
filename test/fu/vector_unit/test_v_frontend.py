...
class TestVectorFrontend(TestCaseWithSimulator):
    def setUp(self):
        ...
        self.announce_mult = MethodMock(i=self.layouts.translator_report_multiplier)
    ...
    def checker(self):
        while len(self.received_mult) + len(self.received_announce) < self.test_number:
            yield
        # be sure that there is no other instructions in pipeline
        yield from self.tick(30)
        self.assertEqual(len(self.received_mult) + len(self.received_announce), self.test_number)
