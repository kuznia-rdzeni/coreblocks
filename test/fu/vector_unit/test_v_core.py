...
class TestVectorCore(TestCaseWithSimulator):
    ...
    def setUp(self):
        ...
        self.gen_params = GenParams(
            test_vector_core_config.replace(
                vector_config=VectorUnitConfiguration(register_bank_count=self.register_bank_count, vrp_count=36)
            )
        )
    ...
    def rob_peek_process(self):
        if self.instr_q:
            ...
            return {
                "rob_data": {"rp_dst": rob_oldest["rp_dst"], "rl_dst": rob_oldest["rp_dst"]},
                "rob_id": rob_oldest["rob_id"],
                "exception": 0,
            }
        else:
    ...
    def generate_input(self):
        instr, vtype = self.generator(
            self.gen_params,
            self.vxrs_layouts.data_layout,
            support_vector=True,
        ...
        for ended in self.instr_ended_q:
            ...
            if current["rp_dst"]["id"] == 0 and current["rp_dst"]["type"] == RegisterType.V:
                ...
                for field in ["rp_s1", "rp_s2", "rp_dst"]:
                    ...
                    self.assertNotEqual(
                        earlier_ended_instr[field],
                        current["rp_dst"],
                        f"{field}; head rob_id: {rob_id}, ended rob_id: {ended['rob_id']}",
                    )
