...
# class AlwaysActiveElaboratable(Elaboratable):
#    def __init__(self):
#        self.method = Method()
#
...
#        return m
#
# class TestPipelineCircuit(Elaboratable):
#    def __init__(self):
#        self.methods = [Method(i=[("n", 1)], o=[("out", 4)]) for i in range(3)]
#        self.subs = [AlwaysActiveElaboratable() for i in range(2)]
#
#    def elaborate(self, platform):
#        m = TModule()
#        m.submodules.subs = ModuleConnector(*self.subs)
#
#        @loop_def_method(m, self.methods)
#        def _(_, n):
#            s = Signal(4)
#            for j in condition_switch(m,n,2,nonblocking = False):
#                self.subs[j].method(m)
#                m.d.comb += s.eq(j)
#            return {"out": s}
#
#        return m
#
# class TestPipeline(TestCaseWithSimulator):
#    def setUp(self):
#        self.circ = SimpleTestCircuit(TestPipelineCircuit())
#
#    def generator(self, k):
#        def process():
#            for i in range(100):
#                yield from self.circ.methods[k].call(n=random.randrange(2))
#        return process
#
#    def test(self):
#        with self.run_simulation(self.circ) as sim:
#            for i in range(3):
#                sim.add_sync_process(self.generator(i))
