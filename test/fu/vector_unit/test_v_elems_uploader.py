...
class TestVectorElemsUploader(TestCaseWithSimulator):
    ...
    def setUp(self):
        ...
        self.circ = SimpleTestCircuit(VectorElemsUploader(self.gen_params, self.write.get_method(), self.read_old_dst.get_method(), self.read_mask.get_method(), self.report_end.get_method()))
        self.m = ModuleConnector(circ=self.circ, write=self.write, read_old_dst = self.read_old_dst, read_mask = self.read_mask, report_end = self.report_end)
