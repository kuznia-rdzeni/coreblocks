...
def pytest_generate_tests(metafunc : pytest.Metafunc):
    ...
    all_tests = load_regression_tests() #The list has to be always in the samo order (e.g. sorted) to allow for parallel testing
    ...
    if {"test_name","backend", "traces", "verbose"}.issubset(metafunc.fixturenames):
        metafunc.parametrize(["test_name","backend", "traces", "verbose"], [(test_name, backend, traces, verbose) for test_name in all_tests])
