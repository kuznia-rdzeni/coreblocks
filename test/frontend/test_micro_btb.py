...
from coreblocks.params import BranchPredictionConfig, GenParams
...
class TestMicroBTB(TestCaseWithSimulator):
    ...
    def setup(self, fixture_initialize_testing_env):
        ...
        self.gen_params = GenParams(
            configurations.test.replace(fetch_block_bytes_log=4, bpu_config=BranchPredictionConfig(ubtb_entries_log=2))
        )
        self.num_entries = 2**self.gen_params.bpu_config.ubtb_entries_log
        self.useful_max = 2**self.gen_params.bpu_config.ubtb_useful_cnt_width - 1
        self.btb = SimpleTestCircuit(MicroBTB(self.gen_params))
    ...
    async def train(self, sim: TestbenchContext, pc, target, taken, cfi_idx=0, cfi_type=CfiType.BRANCH):
        await self.btb.update.call(sim, pc=pc, target=target, cfi_idx=cfi_idx, cfi_type=cfi_type, taken=taken)
    ...
    def test_prediction_carries_position_and_type(self):
        ...
        async def proc(sim: TestbenchContext):
            await self.train(sim, pc, target=0x2000, taken=1, cfi_idx=2, cfi_type=CfiType.JAL)
    ...
    def test_useful_counter_hysteresis(self):
        ...
        async def proc(sim: TestbenchContext):
            ...
            for _ in range(self.useful_max - 1):
                await self.train(sim, pc, target=0, taken=0)
                assert (await self.lookup(sim, pc))["hit"] == 1
    ...
    def test_not_useful_entry_is_reused(self):
        ...
        async def proc(sim: TestbenchContext):
            ...
            for _ in range(self.useful_max):
                await self.train(sim, blocks[0][0], target=0, taken=0)
