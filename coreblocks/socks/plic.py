...
class PlicPeriph(Component, SocksPeripheral):
    ...
    def __init__(
        ...
        *,
        ...
        self.interrupt_count = interrupt_count + 1
        super().__init__(
            {
                "bus": In(WishboneInterface(wb_params).signature),
                "interrupts": In(self.interrupt_count),
                "eip": Out(context_count),
            }
        )
        self.context_count = context_count
        self.priority_bits = priority_bits
        self.priority_count = 2**priority_bits
    ...
    def elaborate(self, platform):
        ...
        enable = Array([Signal(self.interrupt_count) for _ in range(self.context_count)])
        ...
        for priority in range(MIN_PRIORITY, self.priority_count):
            per_interrupt = [
        ...
        eip_comb = []
        per_context_selected_priority_pendings = Array(
            [Signal(self.interrupt_count) for _ in range(self.context_count)]
        )
        ...
        claim_response = Signal(StructLayout({"value": self.bus.dat_r.shape(), "set": 1}))
        ...
        with m.If(claim_response.set):
            m.d.comb += self.bus.dat_r.eq(claim_response.value)
            m.d.comb += self.bus.ack.eq(1)
            m.d.comb += self.bus.err.eq(0)
            m.d.sync += claim_response.set.eq(0)
        ...
        for interrupt in range(self.interrupt_count):
            gen_memory_mapped_register(m, self, OFFSET_SOURCE_PRIORITY + 4 * interrupt, interrupt_priority[interrupt])
        gen_memory_mapped_register(m, self, OFFSET_PENDING_BIT, pending, read_only=True)

        for context in range(self.context_count):
            gen_memory_mapped_register(m, self, OFFSET_ENABLE_BIT + context * (MAX_INTERRUPTS // 8), enable[context])
            gen_memory_mapped_register(
                m,
                self,
                OFFSET_CONTEXT_REGS + context * CONTEXT_REGS_SIZE + OFFSET_CONTEXT_PRIORITY_TRESHOLD,
