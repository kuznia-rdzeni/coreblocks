...
from coreblocks.socks.peripheral import SocksPeripheral, convert_to_wishbone_addr, gen_memory_mapped_register, is_perpiheral_request
...
class PlicPeriph(Component, SocksPeripheral):
    ...
    def elaborate(self, platform):
        ...
        for priority in range(MIN_PRIORITY, PRIORITY_COUNT):
            per_interrupt = [pending[interrupt] & (interrupt_priority[interrupt] == priority) for interrupt in range(self.interrupt_count)]
        ...
        claim_complete_for_context = Signal(StructLayout({"context": range(self.context_count), "claim": 1, "complete": 1, "complete_interrupt": range(self.interrupt_count)}))
        claim_resonse = Signal(StructLayout({"value": self.bus.dat_r.shape(), "set": 1}))
        
        for context in range(self.context_count):
            claim_addr = convert_to_wishbone_addr(self.bus, self.base_addr + OFFSET_CONTEXT_REGS + context * CONTEXT_REGS_SIZE + OFFSET_CONTEXT_INTERRUPT_CLAIM)
            with m.If(is_perpiheral_request(self) & (self.bus.adr == claim_addr)):
                m.d.comb += assign(claim_complete_for_context, {"context": context, "claim": ~self.bus.we, "complete": self.bus.we, "complete_interrupt": self.bus.dat_w})
        ...
        for interrupt in range(self.interrupt_count):
            gen_memory_mapped_register(m, self, self.base_addr + OFFSET_SOURCE_PRIORITY + 4*interrupt , interrupt_priority[interrupt])
        gen_memory_mapped_register(m, self, self.base_addr + OFFSET_PENDING_BIT, pending)
        
        for context in range(self.context_count):
            gen_memory_mapped_register(m, self, self.base_addr + OFFSET_ENABLE_BIT + context*(MAX_INTERRUPTS//8), enable[context])
            gen_memory_mapped_register(m, self, self.base_addr + OFFSET_CONTEXT_REGS + context*CONTEXT_REGS_SIZE + OFFSET_CONTEXT_PRIORITY_TRESHOLD, context_treshold[context])
