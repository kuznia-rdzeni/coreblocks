...
from collections.abc import Sequence, Collection, Iterable, Callable, Mapping, Iterator
...
class TransactionManager(Elaboratable):
    ...
    def _simultaneous(self):
        ...
        for elem in method_map.methods_and_transactions:
            all_sims = frozenset[TransactionOrMethod]().union(*frozenset().union(*elem.simultaneous_list))
            elem.relations = list(
                filterfalse(
        ...
        for elem in method_map.methods_and_transactions:
            ...
            for sims in elem.simultaneous_list:
                for sim in sims:
        ...
        for group in final_simultaneous:
            if any(
                g1 != g2 and not g1.issubset(g2) and not g2.issubset(g1) and g1.issubset(group) and g2.issubset(group)
                for conflict in conflicts
                for g1 in conflict
                for g2 in conflict
            ):
...
class TransactionBase(Owned):
    ...
    def __init__(self):
        ...
        self.simultaneous_list: list[frozenset[frozenset[TransactionOrMethod]]] = []
    ...
    def simultaneous_groups(self, *groups: Collection[TransactionOrMethod]):
        ...
        self.simultaneous_list.append(sgroups)
