...
class TransactionManager(Elaboratable):
    ...
    def _conflict_graph(self):
        ...
        This function generate graph of conflict transactions. It uses
        information passed both explicit to `TransactionManager` using
        function `add_conflict` and implicit using data stored in
        `self.transactions`, `self.methods`.
        Returns
        ----------
        gr : Dict[Set[Transaction]]
            Graph of conflicts between transactions.
...
class Method:
    ...
    simultaneously, at most one of them is granted by the
    ``TransactionManager``, and the rest of them must wait.
    Calling a ``Method`` always takes a single clock cycle.

    Data is combinationaly transferred between ``Method``s and
    ...
    def body(self, m: Module, *, ready=C(1), out=C(0, 0)):
        ...
        ready : Signal, in
            ...
            Assigned combinationaly to the ``ready`` attribute.
        out : Record, in
            Data generated by the ``Method``, which will be passed to
            the calling ``Transaction``. Assigned combinationaly to
