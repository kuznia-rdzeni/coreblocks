...
__all__ = ["PipelinedUnsignedMul"]


class PipelinedMul(Elaboratable):
    """
    PipelinedMul uses pipelining mechanism in order to reduce the time needed to multiply multiple pairs of numbers.
    ...
    def elaborate(self, platform=None):
        ...
        with m.If(~self.valid | self.getting_result):
            ...
            for i in range(self.number_of_multiplications):
                ...
                with Transaction().always_body(m, ready=(self.step == run_on_step)):
...
class PipelinedUnsignedMul(MulBaseUnsigned):
    ...
    def elaborate(self, platform):
        m = TModule()
        m.submodules.mul = mul = PipelinedMul(self.dsp_width, self.dsp_number, self.gen_params.isa.xlen)
