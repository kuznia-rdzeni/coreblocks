    ...
    def elaborate(self, platform):
        ...
        with OneHotSwitch(m, decoder.decode_fn) as OneHotCase:
                    ...
                    for eew_iter in EEW:
                        ...
                        if eew_to_bits(eew_iter) <= self.v_params.elen:
                            with m.Case(eew_iter):
                                ...
                                self.create_flexible_elementwise_function(
                                    m,
                                    f"flexible_sra_{eew_iter_bits}",
                                    (lambda width: lambda x, y: x.as_signed() >> (y[: log2_int(width)]))(eew_iter_bits),
                                )  # TODO optimise number of FlexibleElementWiseFunction's creates
            ...
            with OneHotCase(VectorAluFn.Fn.SLE):  # Set if less or equal than (signed)
                self.create_flexible_elementwise_function(
                    m, "flexible_sle", lambda x, y: Mux(x.as_signed() <= y.as_signed(), 1, 0)
                )
            ...
            with OneHotCase(VectorAluFn.Fn.SLT):  # Set if less than (signed)
                self.create_flexible_elementwise_function(
                    m, "flexible_slt", lambda x, y: Mux(x.as_signed() < y.as_signed(), 1, 0)
                )
            ...
            with OneHotCase(VectorAluFn.Fn.MIN):
                self.create_flexible_elementwise_function(
                    m, "flexible_min", lambda x, y: Mux(x.as_signed() < y.as_signed(), x, y)
                )
            ...
            with OneHotCase(VectorAluFn.Fn.MAX):
                self.create_flexible_elementwise_function(
                    m, "flexible_max", lambda x, y: Mux(x.as_signed() > y.as_signed(), x, y)
                )
