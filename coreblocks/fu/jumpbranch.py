...
class JumpBranchFuncUnit(FuncUnit, Elaboratable):
    def __init__(self, gen_params: GenParams, jb_fn=JumpBranchFn()):
        ...
        self.perf_jumps = HwCounter(
            self.gen_params, "backend.fu.jumpbranch.jumps", "Number of jump instructions issued"
        )
        self.perf_branches = HwCounter(
            self.gen_params, "backend.fu.jumpbranch.branches", "Number of branch instructions issued"
        )
        self.perf_misaligned = HwCounter(
            self.gen_params, "backend.fu.jumpbranch.misaligned", "Number of instructions with misaligned target address"
        )
    ...
    def elaborate(self, platform):
        ...
        def _(arg):
            ...
            self.perf_jumps.incr_when(m, is_jump)
            self.perf_branches.incr_when(m, ~is_jump & ~is_auipc)
